...
#Funcao de estabilidade
def stable_function(coeficients, time, stable):
...
def evalRobot(individual):
    ...
    while dt < DELTA_TIME:
        ...
        for i in xrange(len(Body)):
            ...
            joint_movements.append(stable_function(coefs, dt, stable)*control_joints[i])
...
def mutate(individual):
    ...
    for x in individual:
        if random.random()<0.05:
            ind.append(random.random() * COEF_RANGE - COEF_RANGE / 2.0)
        else:
            ind.append(x)
    return ind
